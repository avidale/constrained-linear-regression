...
class ConstrainedLinearRegression(LinearModel, RegressorMixin):
    ...
    def fit(self, X, y, min_coef=None, max_coef=None, initial_beta=None):
        ...
        self.min_coef_ = (
            min_coef if min_coef is not None else np.repeat(-np.inf, X.shape[1])
        )
        self.max_coef_ = (
            max_coef if max_coef is not None else np.repeat(np.inf, X.shape[1])
        )
        if self.nonnegative:
            ...
            for i in range(len(beta)):
                ...
                if self.lasso:
                    ...
                    new_value2 = (
                        ...
                        - (
                            grad[i]
                            + np.sign(prev_value or new_value) * self.lasso * loss_scale
                        )
                        / hessian[i, i]
                        * self.learning_rate
                    )
